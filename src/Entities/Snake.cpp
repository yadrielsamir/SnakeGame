...
void Snake::update() {
    ...
    if(!godmode && (oldHead[0] == -1 || oldHead[0] == boardSizeWidth || oldHead[1] == -1 || oldHead[1] == boardSizeHeight)) {
            crashed = true;
            return;
    }
    
    if(godmode){
        if(oldHead[0] == 0 && this->direction == LEFT){
            return;
        }
        else if(oldHead[0] == boardSizeWidth -1 && this->direction == RIGHT){
            return;
        }
        else if(oldHead[1] == 0 && this->direction == UP){
            return;

        }
        else if(oldHead[1] == boardSizeHeight - 1 && this->direction == DOWN){
            return;
        }
    }
...
void Snake::draw() {
    for (int i = 0; i < body.size(); i++) {
        ...
        ofSetColor(ofColor::blue);              //sets body to blue
        if (i == 0) {                          
        ofSetColor(ofColor::red);               //sets head to red
        }
